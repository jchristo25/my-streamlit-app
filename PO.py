...
#     # ==========================================
#     # LOGIKA CONDITIONAL FORMATTING (WARNA)
#     # Merah (#D62728) jika > 14 hari, Hijau (#2CA02C) jika <= 14 hari
#     # ==========================================
#     batas_hari = 14
#     color_po     = "#D62728" if avg_app_po > batas_hari else "#2CA02C"
#     color_po_bp  = "#D62728" if avg_po_bp > batas_hari else "#2CA02C"
#     color_sj     = "#D62728" if avg_app_sj > batas_hari else "#2CA02C"
#     color_proses = "#D62728" if avg_proses_sj > batas_hari else "#2CA02C"

#     # 2. PERSIAPAN DATA UNTUK CARD
#     cards_per_row = 4
#     data_cards = [
#         {"title": "Approval PO", "value": f"{avg_app_po:.2f} Hari", "desc": "Waktu pembuatan hingga approval PO", "color": color_po},
#         {"title": "PO hingga BP", "value": f"{avg_po_bp:.2f} Hari", "desc": "PO Approved hingga BP diterbitkan", "color": color_po_bp},
#         {"title": "Approval SJ", "value": f"{avg_app_sj:.2f} Hari", "desc": "Waktu pembuatan hingga approval SJ", "color": color_sj},
#         {"title": "SJ Appr ➔ Close", "value": f"{avg_proses_sj:.2f} Hari", "desc": "SJ Approved hingga status Closed", "color": color_proses}
#     ]
...
#     .card h2 {
#         margin: 0;
#         /* Warna default dihapus karena akan ditimpa oleh inline style (Merah/Hijau) */
#         font-size: 28px;
#         font-weight: 700;
#     }
#     .card p {
...
#         .card { background-color: #262730; border-color: #333; }
#         .card h4 { color: #eeeeee; }
#         .card p { color: #aaaaaa; }
#     }
#     </style>
#     """, unsafe_allow_html=True)

#     # 4. BUILD HTML CARD LIST DENGAN WARNA DINAMIS
#     html_cards = ""
#     for item in data_cards:
#         # Perhatikan penambahan style="color: {item['color']};" di tag <h2>
#         html_cards += f"""
#         <div class="card">
#             <h4>{item['title']}</h4>
...
#     st.markdown(
#         f"""
#         <div class="card-container" style="grid-template-columns: repeat({cards_per_row}, 1fr);">
#             {html_cards}
#         </div>
#         """,
#         unsafe_allow_html=True
...
def tampilkan_kpi_dashboard(po_raw, merged, sj_raw, bp_raw, kolom_pilihan):
    st.header("Ringkasan Performa")
    ...
    # --- PERHITUNGAN UNTUK CARD TRACKING & ALERT ---
    # 1. Hitung PO Belum ada BP (Outstanding)
    outstanding_bp_count = len(merged[
        (merged['PO_APPROVED_ON'].notnull()) & 
        (merged['BP_CREATED_ON'].isna())
    ].drop_duplicates(subset=['PO_NO']))

    # 2. Hitung PO Ekstrem (> 50 BP)
    po_counts = bp_raw['PO_NO'].value_counts()
    extreme_po_count = len(po_counts[po_counts > 50])
    ...
    color_sj     = "#D62728" if avg_app_sj > batas_hari else "#2CA02C"
    color_proses = "#D62728" if avg_proses_sj > batas_hari else "#2CA02C"
    color_outstanding = "#FFA500" if outstanding_bp_count > 0 else "#2CA02C" # Oranye jika ada, Hijau jika 0
    color_extreme = "#D62728" if extreme_po_count > 0 else "#2CA02C" # Merah jika ada, Hijau jika 0

    # --- PERSIAPAN DATA UNTUK CARD ---
    cards_per_row = 4
    data_cards = [
        {"title": "Approval PO", "value": f"{avg_app_po:.2f} Hari", "desc": "Waktu pembuatan hingga approval PO", "color": color_po},
        {"title": "PO hingga BP", "value": f"{avg_po_bp:.2f} Hari", "desc": "PO Approved hingga BP diterbitkan", "color": color_po_bp},
        {"title": "Approval SJ", "value": f"{avg_app_sj:.2f} Hari", "desc": "Waktu pembuatan hingga approval SJ", "color": color_sj},
        {"title": "SJ Appr ➔ Close", "value": f"{avg_proses_sj:.2f} Hari", "desc": "SJ Approved hingga status Closed", "color": color_proses},
        {"title": "PO Pending BP", "value": f"{outstanding_bp_count}", "desc": "PO Approved tanpa BP", "color": color_outstanding},
        {"title": "Alert: PO Ekstrem", "value": f"{extreme_po_count}", "desc": "PO dengan > 50 BP", "color": color_extreme}
    ]
    ...
    .card h4 { margin-top: 0; margin-bottom: 10px; color: #555; font-size: 16px; font-weight: 600; }
    .card h2 { margin: 0; font-size: 28px; font-weight: 700; }
    .card p { font-size: 12px; color: #888; margin-top: 10px; margin-bottom: 0; line-height: 1.4; }
    @media (prefers-color-scheme: dark) {
        .card { background-color: #262730; border-color: #333; }
        .card h4 { color: #eeeeee; }
        .card p { color: #aaaaaa; }
    ...
    # ==========================================
    # INTEGRASI TOMBOL TOGGLE DETAIL ANALISIS
    # ==========================================
    if 'bagian_aktif' not in st.session_state:
        st.session_state.bagian_aktif = None

    if 'bagian_aktif_tab2' not in st.session_state:
        st.session_state.bagian_aktif_tab2 = None
        
    if 'bagian_aktif_tab3' not in st.session_state:
        st.session_state.bagian_aktif_tab3 = None

    st.write("#### Tampilkan Detail Data")
    col1, col2, col3, col4, col5, col6 = st.columns(6)

    with col1:
        st.button("Approval PO", use_container_width=True, key="btn_po", 
                  on_click=toggle_bagian, args=("Approval_PO",))
    with col2:
        st.button("PO hingga BP", use_container_width=True, key="btn_bp", 
                  on_click=toggle_bagian, args=("PO_to_BP",))
    with col3:
        st.button("Approval SJ", use_container_width=True, key="btn_sj", 
                  on_click=toggle_bagian, args=("Approval_SJ",))
    with col4:
        st.button("SJ Appr ➔ Close", use_container_width=True, key="btn_sjclose", 
                  on_click=toggle_bagian, args=("SJ_to_Close",))
    with col5:
        st.button("PO Pending BP", use_container_width=True, key="btn_pending_BP", 
                  on_click=toggle_bagian, args=("pending_BP",))
    with col6:
        st.button("Alert: PO Ekstrem", use_container_width=True, key="btn_PO_extreme", 
                  on_click=toggle_bagian, args=("PO_extreme",))      
    

    st.divider()

    # --- MENAMPILKAN KONTEN HALAMAN ---
    if st.session_state.bagian_aktif == "Approval_PO":
        tampilkan_analisis_po(po_raw=po_raw, kolom_pilihan=kolom_pilihan)
    elif st.session_state.bagian_aktif == "PO_to_BP":
        analisis_durasi_po_ke_bp(merged=merged, bp_raw=bp_raw)
    elif st.session_state.bagian_aktif == "Approval_SJ":
        # Pastikan fungsi ini didefinisikan untuk menerima dan mengembalikan dataframe jika kamu melakukan re-assign
        sj_raw = analisis_durasi_approval_sj(sj_raw=sj_raw) 
    elif st.session_state.bagian_aktif == "SJ_to_Close":
        sj_raw = analisis_durasi_proses_sj(sj_raw=sj_raw)
    elif st.session_state.bagian_aktif == "pending_BP":
        deteksi_po_ekstrem(bp_raw=bp_raw)
    elif st.session_state.bagian_aktif == "PO_extreme":
        tracking_po_tanpa_bp(po_raw=po_raw, bp_raw=bp_raw)

def toggle_bagian_tab2(nama_bagian):
    # Jika bagian yang diklik sudah aktif, maka tutup (None)
    # Jika bagian lain yang diklik, ganti yang aktif agar yang lama tertutup
    if st.session_state.bagian_aktif_tab2 == nama_bagian:
        st.session_state.bagian_aktif_tab2 = None
    else:
        st.session_state.bagian_aktif_tab2 = nama_bagian

def toggle_bagian_tab3(nama_bagian):
    if st.session_state.bagian_aktif_tab3 == nama_bagian:
        st.session_state.bagian_aktif_tab3 = None
    else:
        st.session_state.bagian_aktif_tab3 = nama_bagian
...
def top_10_barang_bsm_by_kategori(bsm_raw, file_consumable, file_nonconsumable):
    st.header("🏆 Top 10 Penggunaan Barang di BSM Terbanyak (Berdasarkan Approval)")
    if file_consumable is None or file_nonconsumable is None:
        st.warning("⚠️ Mohon upload file Master Consumable dan Non-Consumable di sidebar terlebih dahulu untuk melihat analisis ini.")
        return
...
def analisis_pengeluaran_abc(bsm_raw):
    ...
    * **Kelas C:** Barang yang menyerap ~5% dari total pengeluaran (Fokus rendah, biasanya pernak-pernik murah).
    ...
    fig_tree = px.treemap(
        ...
        color_discrete_map={
            'A (High Value)': "#FFFFFF",
            'B (Medium Value)': '#FECB52',
            'C (Low Value)': '#00CC96'
        },
        title="Top 50 Barang Berdasarkan Pengeluaran"
    )

    # 1. TAMBAHKAN uniformtext DI SINI UNTUK MEMAKSA UKURAN SERAGAM
    fig_tree.update_layout(
        height=800, 
        margin=dict(t=50, l=10, r=10, b=10),
        uniformtext=dict(minsize=25, mode='show') 
    )

    # 2. update_traces TETAP DIGUNAKAN UNTUK MENGATUR FORMAT LABEL
    ...
    buffer = BytesIO()
    df_treemap = df_spend.head(50).copy()
    # Tulis dataframe ke dalam buffer sebagai file Excel
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        df_treemap.to_excel(writer, sheet_name='Top 50 Treemap', index=False)
    
    # Tampilkan tombol download di Streamlit
    st.download_button(
        label="📥 Download Data Treemap (Excel)",
        data=buffer.getvalue(),
        file_name="Data_Top_50_Pengeluaran_ABC.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        help="Download detail data 50 barang dengan pengeluaran terbesar yang tampil di grafik Treemap."
    )
    # ==========================================
...
def toggle_bagian(nama_bagian):
    if st.session_state.bagian_aktif == nama_bagian:
        st.session_state.bagian_aktif = None
    else:
        st.session_state.bagian_aktif = nama_bagian
#========================================================================
#========================================================================
#                           Main aplikasi
#========================================================================
...
if uploaded_file is not None:
    ...
    if data is not None:
        ...
        merged = pd.merge(po_raw, bp_raw, on="PO_NO", how="left")
        df_bersih = merged[merged['BP_CREATED_ON'].notnull()].copy()
        df_bersih['durasi_prosesBP'] = df_bersih['BP_CREATED_ON'] - df_bersih['PO_APPROVED_ON']
        ...
        tampilkan_kpi_dashboard(
            po_raw=po_raw, 
            merged=merged, 
            sj_raw=sj_raw, 
            bp_raw=bp_raw,         # Tambahan baru
            kolom_pilihan=kolom_pilihan
        )
        ...
        with tab2:
            st.write("#### 📑 Menu Detail Analisis BP")
            
            # --- 1. PERHITUNGAN UNTUK KPI CARD ---
            # Hitung BP OPEN
            bp_open_count = 0
            if 'STATUS' in bp_raw.columns:
                bp_open_count = len(bp_raw[bp_raw['STATUS'].astype(str).str.strip().str.upper() == 'OPEN'])
            
            # Hitung PO Multi BP
            po_counts = bp_raw['PO_NO'].value_counts()
            multi_bp_count = len(po_counts[po_counts > 1])

            # Penentuan Warna (Merah jika ada masalah, Hijau jika aman)
            color_bp_open = "#D62728" if bp_open_count > 0 else "#2CA02C"
            color_multi_bp = "#FFA500" if multi_bp_count > 0 else "#2CA02C" # Oranye untuk Multi BP

            # --- 2. CSS STYLING UNTUK CARD TAB 2 ---
            st.markdown("""
            <style>
            .kpi-card-tab2 {
                background-color: #ffffff;
            ...
            .kpi-card-tab2 h4 { margin-top: 0; margin-bottom: 10px; color: #555; font-size: 16px; font-weight: 600; }
            .kpi-card-tab2 h2 { margin: 0; font-size: 28px; font-weight: 700; }
            .kpi-card-tab2 p { font-size: 12px; color: #888; margin-top: 10px; margin-bottom: 0; line-height: 1.4; }
            @media (prefers-color-scheme: dark) {
                .kpi-card-tab2 { background-color: #262730; border-color: #333; }
                .kpi-card-tab2 h4 { color: #eeeeee; }
                .kpi-card-tab2 p { color: #aaaaaa; }
            }
            </style>
            """, unsafe_allow_html=True)
            ...
            with col_card1:
                # Render Card HTML
                st.markdown(f"""
                <div class="kpi-card-tab2">
                    <h4>Tracking BP Status OPEN</h4>
                    <h2 style="color: {color_bp_open};">{bp_open_count} BP</h2>
                    <p>Bukti Penerimaan yang masih menggantung</p>
                </div>
                """, unsafe_allow_html=True)
                
                # Tombol persis di bawah card
                st.button("🔍 Klik untuk menampilkan data", key="btn_bp_open", 
                          on_click=toggle_bagian_tab2, args=("BP_OPEN",), use_container_width=True)
                
            with col_card2:
                # Render Card HTML
                st.markdown(f"""
                <div class="kpi-card-tab2">
                    <h4>Deteksi PO Multi BP</h4>
                    <h2 style="color: {color_multi_bp};">{multi_bp_count} PO</h2>
                    <p>Pengiriman barang bertahap / dicicil</p>
                </div>
                """, unsafe_allow_html=True)
                
                # Tombol persis di bawah card
                st.button("⚠️ Klik untuk menampilkan data", key="btn_multi_bp", 
                          on_click=toggle_bagian_tab2, args=("MULTI_BP",), use_container_width=True)
            
            st.divider()
            
            # --- 3. LOGIKA PEMANGGILAN DATA (Tampil HANYA satu yang diklik) ---
            if st.session_state.bagian_aktif_tab2 == "BP_OPEN":
                tracking_bp_open(bp_raw=bp_raw)
                
            elif st.session_state.bagian_aktif_tab2 == "MULTI_BP":
                deteksi_po_multi_bp(bp_raw=bp_raw)
                analisis_perilaku_multi_bp(bp_raw=bp_raw)
                analisis_performa_supplier_bar(df_clean=df_bersih)
        ...
        with tab3:  
            ...
            batas_hari = 14
            sj_telat_count = len(sj_raw[sj_raw['hari_tunggu_app'] > batas_hari])
            
            # Penentuan Warna (Merah jika ada yang telat, Hijau jika 0)
            color_sj_telat = "#D62728" if sj_telat_count > 0 else "#2CA02C"

            # --- 2. MEMBUAT CARD ---
            # Kita bisa memanggil class CSS "kpi-card-tab2" yang sudah didaftarkan di tab 2
            # Gunakan st.columns untuk membatasi lebar card
            col_card_sj, col_kosong = st.columns([1, 1]) 
            
            with col_card_sj:
                st.markdown(f"""
                <div class="kpi-card-tab2">
                    <h4>Alert: Approval SJ Telat</h4>
                    <h2 style="color: {color_sj_telat};">{sj_telat_count} SJ</h2>
                    <p>Melebihi batas waktu {batas_hari} hari</p>
                </div>
                """, unsafe_allow_html=True)
                
                # Tombol persis di bawah card
                st.button("⚠️ Klik untuk menampilkan data telat", key="btn_sj_telat", 
                          on_click=toggle_bagian_tab3, args=("SJ_TELAT",), use_container_width=True)

            st.divider()

            # --- 3. LOGIKA PEMANGGILAN DATA (Tertutup secara default) ---
            if st.session_state.bagian_aktif_tab3 == "SJ_TELAT":
                deteksi_sj_telat(sj_raw=sj_raw, batas_hari=batas_hari)
            
            # --- 4. ANALISIS LAINNYA (Dibiarkan tetap tampil di bagian bawah) ---
            sj_raw = analisis_durasi_approval_sj(sj_raw=sj_raw)
            sj_raw = analisis_durasi_proses_sj(sj_raw=sj_raw)
